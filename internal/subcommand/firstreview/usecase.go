...
func (u *UseCase) Do(ctx context.Context) error {
	...
	for i, summary := range summaries {
		eg.Go(func() error {
			row, err2 := u.measureFirstReviewTime(egCtx, summary)
			if err2 != nil {
				return err2
			}
			resultRows[i] = row
	...
	progressStopFunc()
	if err != nil {
		return fmt.Errorf("failed to measure first review times: %w", err)
	}
	...
	if !header {
		fmt.Println("レビュー反応のあるPRが見つかりませんでした")
	}

	return nil
}

func (u *UseCase) measureFirstReviewTime(ctx context.Context, summary *mygithub.PullRequestSummary) (string, error) {
	...
	duration, ok := detail.TimeToFirstReview()
	if !ok {
		return "", nil
	}
